...
public class PlayerMovementVerTwo : MonoBehaviour
{
    ...
    [Header("Attack Settings")]
    public float baseAttackDamage = 25f; // Sát thương cơ bản
    public float attackDamage; // Sát thương hiện tại
    public float attackRange = 2f;
    public float attackCooldown = 0.5f;
    private float lastAttackTime;
    private int attackHash = Animator.StringToHash("Attack");
    ...
    [Header("Weapon Settings")]
    public List<GameObject> weapons; // Danh sách các vũ khí
    public List<float> weaponDamage; // Sát thương tương ứng của từng vũ khí
    private int currentWeaponIndex = 0; // Vũ khí hiện tại
    ...
    private void SwitchWeapon(int index)
    {
        ...
        // Cập nhật sát thương dựa trên vũ khí được chọn
        attackDamage = weaponDamage[index];
    ...
    private void Attack()
    {
        ...
        foreach (Collider enemyCollider in hitEnemies)
        {
            ...
            if (enemy != null)
            {
                enemy.TakeDamage(attackDamage); // Sử dụng sát thương hiện tại
            }

            Boss_controller boss = enemyCollider.GetComponent<Boss_controller>();
            if (boss != null)
            {
                boss.TakeDamage((int)attackDamage); // Sử dụng sát thương hiện tại
            }
        }
    }
